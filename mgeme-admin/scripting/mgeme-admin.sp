...
public void OnClientDisconnect(int client)
{
    ...
    if (GetClientTime(client) > 5.0) // player wasn't kicked instantly
    {
        char Name[64];
...
public Action QueryDb(Handle timer, DataPack pack)
{
    ...
    if (db != null)
    {
        ...
        else
        {
            ...
            {
                KickClient(client, "One ETF2L match required (no alts)");
            }
...
Action Event_PlayerConnect(Event ev, const char[] name, bool dontBroadcast)
{
...
Action Event_PlayerDisconnect(Event ev, const char[] name, bool dontBroadcast)
{
   SetEventBroadcast(ev, true);
   ev.BroadcastDisabled = true;
   return Plugin_Continue;
...
Action Event_PlayerChangeName(Event ev, const char[] name, bool dontBroadcast)
{
  SetEventBroadcast(ev, true);
  ev.BroadcastDisabled = true;
  ...
  ev.GetString("oldname", oldname, sizeof(oldname));
